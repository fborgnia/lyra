...
class MemoryInjectionLayer(nn.Module):
    ...
    def forward(self, gnn, memory_graph, tokenizer, current_input_ids, current_attention_mask):
        if not memory_graph:
            print("No memory to inject.", file=sys.stdout)
            return current_input_ids, current_attention_mask

        best_memory_index = gnn(memory_graph, current_input_ids)
        ...
        retrieved_memory_ids = memory_graph[best_memory_index]["input_ids"].to(device)
